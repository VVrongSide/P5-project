...
Dynamic Source Routing (WDSR) protocol is a reactive routing protocol designed specifically
...
WDSR Routing Overview
********************

This model implements the base specification of the Dynamic Source Routing
(WDSR) protocol. Implementation is based on :rfc:`4728`, with some extensions
and modifications to the RFC specifications.

WDSR operates on a on-demand behavior. Therefore, our WDSR model buffers all
packets while a route request packet (RREQ) is disseminated. We implement
a packet buffer in wdsr-rsendbuff.cc. The packet queue implements
...
than in its absence.  WDSR is able to
...
WDSR operates with direct access to IP header, and operates between network
and transport layer.  When packet is sent out from transport layer, it
passes itself to WDSR and WDSR header is appended.
...
* The WDsrFsHeader has added 3 fields: message type, source id, destination id, and these changes only for post-processing
...
* Route Reply header is not word-aligned in WDSR RFC, change it to word-aligned in implementation
* WDSR works as a shim header between transport and network protocol, it needs its own forwarding mechanism, we are changing the packet transmission to hop-by-hop delivery, so we added two fields in wdsr fixed header to notify packet delivery
...
The following should be kept in mind when running WDSR as routing protocol:
...
To have a node run WDSR, the easiest way would be to use the WDsrHelper
and WDsrMainHelpers in your simulation script. For instance:
...
The example scripts inside ``src/wdsr/examples/`` demonstrate the use of WDSR based nodes in different scenarios.
The helper source can be found inside ``src/wdsr/helper/wdsr-main-helper.{h,cc}``
and ``src/wdsr/helper/wdsr-helper.{h,cc}``

Examples
********

The example can be found in ``src/wdsr/examples/``:

* wdsr.cc use WDSR as routing protocol within a traditional MANETs environment[3].

WDSR is also built in the routing comparison case in ``examples/routing/``:
...
* Unit tests have been written to verify the internals of WDSR. This can be found in ``src/wdsr/test/wdsr-test-suite.cc``. These tests verify whether the methods inside WDSR module which deal with packet buffer, headers work correctly.
* Simulation cases similar to [3] have been tested and have comparable results.
* manet-routing-compare.cc has been used to compare WDSR with three of other routing protocols.
...
The model is not fully compliant with :rfc:`4728`. As an example, WDsr fixed size header
has been extended and it is four octets longer then the RFC specification.
As a consequence, the WDSR headers can not be correctly decoded by Wireshark.
...
[1] Original paper: https://web.archive.org/web/20150430233910/http://www.monarch.cs.rice.edu/monarch-papers/wdsr-chapter00.pdf
