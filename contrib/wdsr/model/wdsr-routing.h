...
#ifndef WDSR_ROUTING_H
#define WDSR_ROUTING_H

#include "wdsr-errorbuff.h"
...
/**
 * \class WDsrRouting
 * \brief WDsr Routing base
 */
class WDsrRouting : public IpL4Protocol
{
  public:
    /**
    ...
    /**
     * \brief Define the wdsr protocol number.
     */
     ...
     */
    void SetRouteCache(Ptr<wdsr::WDsrRouteCache> r);
    /**
     * \brief Get the route cache.
     * \return the route cache
     */
    Ptr<wdsr::WDsrRouteCache> GetRouteCache() const;
    /**
     * \brief Set the node.
     * \param r the request table to set
     */
    void SetRequestTable(Ptr<wdsr::WDsrRreqTable> r);
    /**
     * \brief Get the request table.
     * \return the request table
     */
    Ptr<wdsr::WDsrRreqTable> GetRequestTable() const;
    /**
     * \brief Set the node.
     * \param r the passive buffer to set
     */
    void SetPassiveBuffer(Ptr<wdsr::WDsrPassiveBuffer> r);
    /**
     * \brief Get the passive buffer
     * \return the passive buffer
     */
    Ptr<wdsr::WDsrPassiveBuffer> GetPassiveBuffer() const;

    /**
     * \brief Checks if the link is cached in the route cache
     * See also WDsrRouteCache::IsLinkCache
     *
     ...
     * data packet. See also WDsrRouteCache::UpdateRouteEntry
     *
     ...
     * See also WDsrRreqTable::FindSourceEntry
     *
    ...
    /**
     * \brief Get the wdsr protocol number.
     ...
     */
    uint32_t GetPriority(WDsrMessageType messageType);
    /**
    ...
    void SendUnreachError(Ipv4Address unreachNode,
                          ...
                          uint8_t protocol); /**
                                              ...
                                              */
    void ForwardErrPacket(WDsrOptionRerrUnreachHeader& rerr,
     ...
     */
    bool SendRealDown(WDsrNetworkQueueEntry& newEntry);
    /**
     ...
     * \param protocol number
     */
    void SendPacketFromBuffer(const WDsrOptionSRHeader& sourceRoute,
                              Ipv4Address nextHop,
                              uint8_t protocol);
    /**
     ...
     */
    void CancelPacketAllTimer(WDsrMaintainBuffEntry& mb);
    /**
     ...
     */
    void ScheduleLinkPacketRetry(WDsrMaintainBuffEntry& mb, uint8_t protocol);
    /**
     * \brief Schedule the packet retransmission based on passive acknowledgment
     * \param mb maintenance buffer entry
     * \param protocol the protocol number
     */
    void SchedulePassivePacketRetry(WDsrMaintainBuffEntry& mb, uint8_t protocol);
    /**
     ...
     * \param protocol the protocol number
     */
    void ScheduleNetworkPacketRetry(WDsrMaintainBuffEntry& mb, bool isFirst, uint8_t protocol);
    /**
     * \brief This function deals with packet retransmission timer expire using link acknowledgment
     * \param mb maintenance buffer entry
     * \param protocol the protocol number
     */
    void LinkScheduleTimerExpire(WDsrMaintainBuffEntry& mb, uint8_t protocol);
    /**
     ...
     */
    void NetworkScheduleTimerExpire(WDsrMaintainBuffEntry& mb, uint8_t protocol);
    /**
     * \brief This function deals with packet retransmission timer expire using passive
     * acknowledgment \param mb maintenance buffer entry \param protocol the protocol number
     */
    void PassiveScheduleTimerExpire(WDsrMaintainBuffEntry& mb, uint8_t protocol);
    /**
     ...
     * \param protocol the protocol number
     */
    void SendErrorRequest(WDsrOptionRerrUnreachHeader& rerr, uint8_t protocol);
    /**
    ...
    uint8_t Process(Ptr<Packet>& packet,
                    ...
                    bool& isDropped);
    /**
     * \brief Insert a new WDsr Option.
     * \param option the option to insert
     */
    void Insert(Ptr<wdsr::WDsrOptions> option);
    /**
     * \brief Get the option corresponding to optionNumber.
     * \param optionNumber the option number of the option to retrieve
     * \return a matching WDsr option
     */
    Ptr<wdsr::WDsrOptions> GetOption(int optionNumber);
    /**
  ...
  protected:
    ...
    TracedCallback<const WDsrOptionSRHeader&> m_txPacketTrace; ///< packet trace callback
  ...
  private:
     ...
     * \param protocol The protocol we receive, need to verify it is wdsr protocol
     * \param from The from address we received the packet
     * \param to The address this packet is destined for
     * \param packetType The wdsr packet type, 0 is for control packet, 1 for data packet
    ...
    /**
     * \brief Define the list to hold WDSR options.
     */
    typedef std::list<Ptr<WDsrOptions>> WDsrOptionList_t;
    /**
     * \brief List of WDSR Options supported.
     */
    WDsrOptionList_t m_options;
    ...
    WDsrSendBuffer m_sendBuffer; ///< The send buffer

    WDsrErrorBuffer m_errorBuffer; ///< The error buffer to save the error messages
    ...
    WDsrMaintainBuffer m_maintainBuffer; ///< The declaration of maintain buffer
    ...
    Ptr<wdsr::WDsrRreqTable> m_rreqTable; ///< A "drop-front" queue used by the routing layer to cache
                                        ///< route request sent.

    Ptr<wdsr::WDsrPassiveBuffer> m_passiveBuffer; ///< A "drop-front" queue used by the routing layer
    ...
    std::map<uint32_t, Ptr<wdsr::WDsrNetworkQueue>> m_priorityQueue; ///< priority queues

    WDsrGraReply m_graReply; ///< The gratuitous route reply.

    WDsrNetworkQueue m_networkQueue; ///< The network queue.
