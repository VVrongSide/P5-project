...
#ifndef WDSR_NETWORK_QUEUE_H
#define WDSR_NETWORK_QUEUE_H
...
enum WDsrMessageType
{
    WDSR_CONTROL_PACKET = 1,
    WDSR_DATA_PACKET = 2
};

/**
 * \ingroup wdsr
 * \brief WDSR Network Queue Entry
 */
class WDsrNetworkQueueEntry
{
  public:
    /**
     * Construct a WDsrNetworkQueueEntry with the given parameters
     *
     * \param pa packet
     ...
     * \param r Route
     */
    WDsrNetworkQueueEntry(Ptr<const Packet> pa = nullptr,
     ...
     */
    bool operator==(const WDsrNetworkQueueEntry& o) const
    {
...
class WDsrNetworkQueue : public Object
{
  public:
    /**
    ...
    WDsrNetworkQueue();
    /**
     * Construct a WDsrNetworkQueue with the given
     * maximum length and maximum delay.
     *
     * \param maxLen Maximum queue size
     * \param maxDelay Maximum entry lifetime in the queue
     */
    WDsrNetworkQueue(uint32_t maxLen, Time maxDelay);
    ~WDsrNetworkQueue() override;

    /**
     * Find the packet entry with a given next hop
     * \param nextHop the IP address of the next hop
     * \param entry the WDSR queue entry
     * \returns true if found
     */
    bool FindPacketWithNexthop(Ipv4Address nextHop, WDsrNetworkQueueEntry& entry);
    /**
     ...
     *         false otherwise
     */
    bool Enqueue(WDsrNetworkQueueEntry& entry);
    /**
     ...
     *         false otherwise
     */
    bool Dequeue(WDsrNetworkQueueEntry& entry);
    /**
     * Number of entries
     *
     ...
     * \return the current queue entry
     */
    std::vector<WDsrNetworkQueueEntry>& GetQueue()
    {
        return m_wdsrNetworkQueue;
    }
  ...
  private:
     ...
     */
    void Cleanup();
    std::vector<WDsrNetworkQueueEntry> m_wdsrNetworkQueue; //!< Queue (vector) of entries
