...
NS_LOG_COMPONENT_DEFINE("WDsrErrorBuffer");

namespace wdsr
{

uint32_t
WDsrErrorBuffer::GetSize()
{
    Purge();
...
bool
WDsrErrorBuffer::Enqueue(WDsrErrorBuffEntry& entry)
{
    Purge();
    for (std::vector<WDsrErrorBuffEntry>::const_iterator i = m_errorBuffer.begin();
         i != m_errorBuffer.end();
         ++i)
    {
...
void
WDsrErrorBuffer::DropPacketForErrLink(Ipv4Address source, Ipv4Address nextHop)
{
     ...
     */
    for (std::vector<WDsrErrorBuffEntry>::iterator i = m_errorBuffer.begin();
         i != m_errorBuffer.end();
         ++i)
    {
...
bool
WDsrErrorBuffer::Dequeue(Ipv4Address dst, WDsrErrorBuffEntry& entry)
{
    Purge();
    /*
...
bool
WDsrErrorBuffer::Find(Ipv4Address dst)
{
    /*
...
struct IsExpired
{
     ...
     */
    bool operator()(const WDsrErrorBuffEntry& e) const
    {
...
void
WDsrErrorBuffer::Drop(WDsrErrorBuffEntry en, std::string reason)
{
...
void
WDsrErrorBuffer::DropLink(WDsrErrorBuffEntry en, std::string reason)
{
