...
#ifndef WDSR_PASSIVEBUFF_H
#define WDSR_PASSIVEBUFF_H
...
/**
 * \ingroup wdsr
 * \brief WDSR Passive Buffer Entry
 */
class WDsrPassiveBuffEntry
{
  public:
    /**
     * Construct a WDsrPassiveBuffEntry with the given parameters
     *
     * \param pa packet
     ...
     */
    WDsrPassiveBuffEntry(Ptr<const Packet> pa = nullptr,
     ...
     */
    bool operator==(const WDsrPassiveBuffEntry& o) const
    {
...
/**
 * \ingroup wdsr
 * \class WDsrPassiveBuffer
 * \brief WDSR passive buffer
 */
/************************************************************************************************************************/
class WDsrPassiveBuffer : public Object
{
  public:
    /**
    ...
    bool Enqueue(WDsrPassiveBuffEntry& entry);
    /// Return first found (the earliest) entry for given destination
    /// \param [in] dst Entry destination
    /// \param [out] entry The Entry found (if any).
    /// \return true on success
    bool Dequeue(Ipv4Address dst, WDsrPassiveBuffEntry& entry);
    ...
    bool AllEqual(WDsrPassiveBuffEntry& entry);
    /// Number of entries
  ...
  private:
    ...
    std::vector<WDsrPassiveBuffEntry> m_passiveBuffer;
    /// Remove all expired entries
    void Purge();
    /// Notify that packet is dropped from queue by timeout
    /// \param en BuffEntry Buffer entry
    /// \param reason Drop reason
    void Drop(WDsrPassiveBuffEntry en, std::string reason);
    /// Notify that packet is dropped from queue by timeout
    /// \param en BuffEntry Buffer entry
    /// \param reason Drop reason
    void DropLink(WDsrPassiveBuffEntry en, std::string reason);
    ...
    static bool LinkEqual(WDsrPassiveBuffEntry en, const std::vector<Ipv4Address> link)
    {
