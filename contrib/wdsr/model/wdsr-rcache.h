...
#ifndef WDSR_RCACHE_H
#define WDSR_RCACHE_H
...
/**
 * \class WDsrLinkStab
 * \brief WDsrLinkStab class (WDSR link stability)
 */
class WDsrLinkStab
{
  public:
     ...
     */
    WDsrLinkStab(Time linkStab = Simulator::Now());
    /**
     * \brief Destructor
     */
    virtual ~WDsrLinkStab();
...
/**
 * \class WDsrNodeStab
 * \brief WDsrNodeStab class (WDSR node stability)
 */
class WDsrNodeStab
{
  public:
     ...
     */
    WDsrNodeStab(Time nodeStab = Simulator::Now());
    virtual ~WDsrNodeStab();
...
/**
 * \class WDsrRouteCacheEntry
 * \brief WDsrRouteCacheEntry class for entries in the route cache
 */
class WDsrRouteCacheEntry
{
  public:
     ...
     */
    bool operator==(const WDsrRouteCacheEntry& o) const
    {
...
/**
 * \ingroup wdsr
 * \brief WDSR route request queue
 * Since WDSR is an on demand routing we queue requests while looking for route.
 */
class WDsrRouteCache : public Object
{
  public:
    /**
    ...
    WDsrRouteCache();
    ~WDsrRouteCache() override;

    // Delete assignment operator to avoid misuse
    WDsrRouteCache& operator=(const WDsrRouteCache&) = delete;

    /**
     * \brief Remove the aged route cache entries when the route cache is full
     * \param rtVector the route cache to scan.
     */
    void RemoveLastEntry(std::list<WDsrRouteCacheEntry>& rtVector);
    /**
     * \brief Define the vector of route entries.
     */
    typedef std::list<WDsrRouteCacheEntry::IP_VECTOR> routeVector;
     ...
     */
    void PrintRouteVector(std::list<WDsrRouteCacheEntry> route);
    /**
     ...
     * \return true if same
     */
    bool FindSameRoute(WDsrRouteCacheEntry& rt, std::list<WDsrRouteCacheEntry>& rtVector);
    /**
  ...
  private:
    ...
    uint32_t m_maxCacheLen; ///< The maximum number of packets that we allow a routing protocol to
                            ///< buffer.
    Time RouteCacheTimeout; ///< The maximum period of time that wdsr is allowed to for an unused
                            ///< route.
     ...
     */
    typedef std::list<WDsrRouteCacheEntry> routeEntryVector;
...
#define MAXWEIGHT 0xFFFF;
    ...
    std::map<Ipv4Address, WDsrRouteCacheEntry::IP_VECTOR>
        m_bestRoutesTable_link;                     ///< for link route cache
    std::map<Link, WDsrLinkStab> m_linkCache;        ///< The data structure to store link info
    std::map<Ipv4Address, WDsrNodeStab> m_nodeCache; ///< The data structure to store node info
    /**
     ...
     */
    bool LookupRoute_Link(Ipv4Address id, WDsrRouteCacheEntry& rt);
    /**
  ...
  public:
     ...
     * \return true if the link is cached
     */
    bool AddRoute_Link(WDsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address node);
    /**
