...
main(int argc, char* argv[])
{
  ...
  //LogComponentEnable ("WDsrNetworkQueue", LOG_LEVEL_ALL);
    ...
    cmd.AddValue("comp", "Compare WDSR to DSR, Default: 0", dsr);
    ...
    if (!fixed) {
      ...
      serverApps = sink.Install(adhocNodes);

      serverApps.Start(Seconds(0));
      serverApps.Stop(Seconds(TotalTime));
      while ((txDur*(dataIter+1)) <=TotalTime){

          for (uint32_t i = 0; i < nWifis; i++){
            if ((uint32_t)rand()%2){
                while (true){
                        recepient = (uint32_t)rand()%20;
                        if (i == recepient){
                            continue;
                        }
                        break;
                ...
                UdpEchoClientHelper echoClient(allInterfaces.GetAddress(recepient), port);
                ...
                clientApps.Start(Seconds(txDur*dataIter));
                clientApps.Stop(Seconds(txDur*(dataIter+1)));
            }
        }
        dataIter++;
    }
    ...
    Simulator::Stop(Seconds(TotalTime+4));
    /******* Animation *******/
